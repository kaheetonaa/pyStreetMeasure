...
path="unidepthv2_vits14_simp.onnx"
...
def prepare_input(image):
        ...
        # Resize input image
        input_img = cv2.resize(input_img, (644, 364)) #input requirements
...
outputs=unidepth.run(None,{"image": input_tensor})

out=outputs[1][0].transpose(1,2,0)
out=colorize_map(out)
out=cv2.resize(out,(o_width,o_height))
cv2.imwrite("out-resized.jpg",out)
